...
POSTGRES_RHEL_RPM="pgdg-redhat-repo-latest.noarch.rpm"
...
LOCAL_FILE_SRC_PATH="/home/centos"
...
if [ "$USE_LOCAL_SRC_FILES" == "TRUE" ]; then
    sudo yum install $LOCAL_FILE_SRC_PATH/$POSTGRES_RHEL_RPM -y
   else
    sudo yum install https://download.postgresql.org/pub/repos/yum/reporpms/EL-7-x86_64/pgdg-redhat-repo-latest.noarch.rpm -y
fi

# postgres client and local postgres server install
